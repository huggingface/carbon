...
for i in tqdm(range(0, len(chunks), batch_size)):
    ...
    for j, chunk_text in enumerate(batch_texts):
        ...
        current_seq_preds = pred_tokens[j, :valid_seq_len-1] # Only decode what we need
        decoded_token_list = tokenizer.batch_decode(
            current_seq_preds.reshape(-1, 1), 
        ...
        for t_idx in range(valid_seq_len - 1):
            ...
            if overlap_len > 0:
                ...
                # 4. Compare
                char_correct += sum(1 for p, t in zip(pred_segment, true_segment) if p == t)
