...
def _shard_worker(args):
    """Score one GPU shard using token-level scoring. Returns [(uid, log_likelihood), ...]."""
...
def score_hf(seqs, model, revision, dtype_str, batch_size: int, max_length: int):
    """Multi-GPU token-level scoring across all sequences."""
